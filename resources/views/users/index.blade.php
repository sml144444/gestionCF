...
<div class="page-header">
    <div>
        ...
        <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
            ...
            @if($s)
            @php [$col,$cnt,$lbl] = $s; @endphp
            @if($i > 0)<span style="color:#e2e8f0; font-size:12px;">·</span>@endif
            <span style="font-size:12px; color:#64748b;">
                <span style="font-weight:800; color:{{ $col }};">{{ $cnt }}</span> {{ $lbl }}
            </span>
            @endif
            @endforeach
        </div>
...
{{-- ════ FILTERS ════ --}}
<form method="GET" action="{{ route('users.management.index') }}" class="filter-bar">
...
<div style="background:white; border-radius:14px; border:1px solid #e2e8f0;
    ...
    <div class="table-head">
        @foreach(['Utilisateur','Rôle','Accès Spatie','Modules','Actions'] as $h)
        <span style="font-size:10px; font-weight:700; color:#94a3b8;
                     letter-spacing:1.2px; text-transform:uppercase;">{{ $h }}</span>
        @endforeach
    </div>
    ...
    <div class="table-row">
        ...
        <div class="row-user" style="display:flex; align-items:center; gap:10px; min-width:0;">
            <div style="width:34px; height:34px; border-radius:9px; flex-shrink:0;
                        background:{{ $rc['bg'] }}; display:flex; align-items:center;
                        justify-content:center; font-size:11px; font-weight:800;
                        color:{{ $rc['text'] }};">{{ $initials }}</div>
        ...
        <div class="col-role">
            <span style="display:inline-flex; align-items:center; gap:5px; font-size:11px;
                         font-weight:600; color:{{ $rc['text'] }}; padding:3px 10px;
                         border-radius:99px; background:{{ $rc['bg'] }}; white-space:nowrap;">
                <span style="width:6px; height:6px; border-radius:50%;
                             background:{{ $rc['dot'] }}; flex-shrink:0;"></span>
                {{ ucfirst($user->role) }}
            </span>
        </div>
        ...
        <div class="col-actions" style="display:flex; align-items:center; gap:5px;">
            ...
            <button class="action-btn blue" title="Modifier le rôle"
                    onclick="openRoleModal({{ $user->id }}, '{{ addslashes($user->name) }}', '{{ $spatieRoleNames }}')">
                🛡
            </button>
            ...
            <div style="display:flex; flex-direction:column; gap:7px;">
                ...
                <label class="role-option-label"
                    ...
                    <span style="font-size:9px; font-weight:700; padding:2px 7px; border-radius:99px;
                                 background:{{ $src['bg'] }}; color:{{ $src['text'] }}; white-space:nowrap;">
