...
{{-- ══════════════════════════════════════════════════════════
     ROOT Alpine scope — photo viewer
══════════════════════════════════════════════════════════ --}}
...
<div class="max-w-5xl mx-auto space-y-4">
    ...
    {{-- ══════════════════════════════════════════════════════
         HERO CARD — cover + avatar + name + stats
    ══════════════════════════════════════════════════════ --}}
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">

        {{-- Cover banner --}}
        <div class="h-28 relative overflow-hidden {{ $rc['bg'] }}">
        ...
        <div class="px-6 pb-5">
            ...
            <div class="flex -mt-10 mb-4">
                ...
                <button type="button" @click="photoOpen = true"
                    ...
                    {{-- Gradient ring --}}
                    <span class="absolute inset-0 bg-gradient-to-tr {{ $rc['grad'] }}
                                 opacity-90 group-hover:opacity-100 group-hover:scale-105
                                 transition-all duration-300 -m-[2.5px]
                                 {{ $isCircle ? 'rounded-full' : 'rounded-[20px]' }}"></span>
                    {{-- White gap --}}
                    <span class="absolute inset-0 bg-white -m-[1px] z-[1]
                                 {{ $isCircle ? 'rounded-full' : 'rounded-[18px]' }}"></span>
                    {{-- Photo --}}
                    <span class="relative z-[2] block w-20 h-20 {{ $rc['shape'] }} overflow-hidden shadow-md">
                        <img src="{{ asset('storage/' . $user->photo) }}" alt="{{ $user->name }}"
                             class="w-full h-full object-cover transition-transform duration-300
                                    group-hover:scale-110">
                    </span>
                    {{-- Zoom hint --}}
                    <span class="absolute inset-0 z-[3] {{ $isCircle ? 'rounded-full' : 'rounded-2xl' }}
                ...
                @else
                <div class="w-20 h-20 {{ $rc['shape'] }} {{ $rc['bg'] }} flex items-center justify-center
            ...
            <div class="mb-4">
                ...
                <div class="flex flex-wrap items-center gap-2 mt-2">
                    <span class="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1
                                 rounded-full border {{ $rc['badge'] }} {{ $rc['border'] }}">
                        {{ $rc['icon'] }} {{ $rc['label'] }}
                    </span>
                    @if($isStagiaire && $user->filiere)
                    <span class="text-xs font-medium px-2.5 py-1 rounded-full
                                 bg-blue-50 text-blue-700 border border-blue-100">
                        {{ $user->filiere->name }}
                    </span>
                    @endif
                    @if($isStagiaire && $user->groupe)
                    <span class="text-xs font-medium px-2.5 py-1 rounded-full
                    ...
                    @endif
                    @if(!$isStagiaire && $user->matricule_formateur)
            ...
            <div class="grid grid-cols-3 gap-3 pt-3 border-t border-slate-100">
                ...
                <div class="text-center">
                    <p class="text-lg font-bold text-slate-800">{{ $daysMember }}</p>
                ...
                <div class="text-center border-x border-slate-100">
                    <p class="text-lg font-bold {{ $absCount > 0 ? 'text-red-500' : 'text-emerald-500' }}">
                        {{ $absCount }}
                    </p>
                ...
                {{-- Non justifiées --}}
                <div class="text-center">
                    <p class="text-lg font-bold {{ $absUnjust > 0 ? 'text-amber-500' : 'text-emerald-500' }}">
                        {{ $absUnjust }}
                    </p>
                ...
                {{-- Modules --}}
                <div class="text-center border-x border-slate-100">
                    <p class="text-lg font-bold {{ $rc['text'] }}">{{ $modCount }}</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="px-5 py-3.5 border-b border-slate-100 flex items-center gap-2.5">
                <span class="w-7 h-7 rounded-lg {{ $rc['light'] }} {{ $rc['text'] }}
            ...
            <div class="divide-y divide-slate-50">
                ...
                <div class="flex items-center gap-3 px-5 py-3">
                    ...
                    <div class="flex-1 flex items-center justify-between gap-4 min-w-0">
                        <span class="text-xs text-slate-400 font-medium flex-shrink-0">{{ $field['label'] }}</span>
                        <span class="text-sm font-semibold text-slate-700 truncate text-right">{{ $field['value'] }}</span>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        {{-- ── Scolarité (stagiaire) / Pro info (staff) ─────── --}}
        @if($isStagiaire)
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-3.5 border-b border-slate-100 flex items-center gap-2.5">
                <span class="w-7 h-7 rounded-lg {{ $rc['light'] }} {{ $rc['text'] }}
            ...
            <div class="divide-y divide-slate-50">
                ...
                @endphp
                @foreach($scolFields as $f)
                <div class="flex items-center justify-between px-5 py-3">
                    <span class="text-xs text-slate-400 font-medium">{{ $f['label'] }}</span>
                    <span class="text-sm font-semibold text-slate-700">{{ $f['value'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="divide-y divide-slate-50">
                <div class="flex items-center justify-between px-5 py-3">
                    ...
                    <span class="text-sm font-mono font-semibold text-slate-700">{{ $user->matricule_formateur ?? '—' }}</span>
                </div>
                <div class="flex items-center justify-between px-5 py-3">
                    <span class="text-xs text-slate-400 font-medium">Date d'embauche</span>
                    <span class="text-sm font-semibold text-slate-700">{{ $user->date_embauche?->format('d/m/Y') ?? '—' }}</span>
                </div>
                @if($isFormateur)
                <div class="flex items-center justify-between px-5 py-3">
                    <span class="text-xs text-slate-400 font-medium">Limite heures</span>
                    <span class="text-sm font-semibold {{ $rc['text'] }}">
                        {{ $user->nbr_heure_limit ? $user->nbr_heure_limit . ' h / semaine' : '—' }}
                    </span>
                </div>
                <div class="flex items-center justify-between px-5 py-3">
                    <span class="text-xs text-slate-400 font-medium">Modules assignés</span>
                    <span class="text-sm font-bold {{ $rc['text'] }}">{{ $user->modules->count() }} module(s)</span>
                </div>
                @endif
                <div class="flex items-center justify-between px-5 py-3">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="text-xs font-semibold px-2 py-0.5 rounded-full {{ $rc['light'] }} {{ $rc['text'] }}">
                {{ $user->modules->count() }}
            </span>
        </div>
        <div class="p-5">
            <div class="flex flex-wrap gap-2">
                ...
                <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl
                            {{ $rc['light'] }} border {{ $rc['border'] }}">
                    <span class="text-xs font-semibold {{ $rc['text'] }}">{{ $module->name }}</span>
                    @if($module->nbr_heure)
                    <span class="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-white/70 {{ $rc['text'] }} opacity-70">
                        {{ $module->nbr_heure }}h
                    </span>
                    @endif
                </div>
        ...
        <div class="p-5">
            ...
            <div class="grid grid-cols-3 gap-3 mb-4">
                ...
                <div class="rounded-xl bg-emerald-50 border border-emerald-100 p-3 text-center">
                    <p class="text-xl font-bold text-emerald-600">{{ $absJust }}</p>
                ...
                <div class="rounded-xl bg-amber-50 border border-amber-100 p-3 text-center">
                    <p class="text-xl font-bold text-amber-600">{{ $absUnjust }}</p>
                    <p class="text-[10px] text-amber-400 font-semibold uppercase tracking-wide mt-0.5">Non just.</p>
                </div>
            </div>

            {{-- Justification rate bar --}}
            @php $justRate = round(($absJust / $absCount) * 100); @endphp
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    <span class="text-xs text-slate-400 font-medium">Taux de justification</span>
                    <span class="text-xs font-bold {{ $justRate >= 50 ? 'text-emerald-600' : 'text-amber-600' }}">
                        {{ $justRate }}%
                    </span>
                </div>
                <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-700
                                {{ $justRate >= 50 ? 'bg-emerald-400' : 'bg-amber-400' }}"
                         style="width: {{ $justRate }}%"></div>
                </div>
            </div>
...
{{-- ══════════════════════════════════════════════════════════
     PHOTO VIEWER MODAL — Instagram style
══════════════════════════════════════════════════════════ --}}
@if($user->photo)
<div
    x-show="photoOpen"
...
>
    ...
    >
        ...
        {{-- Gradient ring → white gap → photo --}}
        <div class="p-[3px] shadow-2xl {{ $isCircle ? 'rounded-full' : 'rounded-[28px]' }}
                    bg-gradient-to-tr {{ $rc['grad'] }}">
            <div class="p-[3px] bg-black {{ $isCircle ? 'rounded-full' : 'rounded-[26px]' }}">
                <img src="{{ asset('storage/' . $user->photo) }}" alt="{{ $user->name }}"
                     class="block object-cover shadow-xl
                            {{ $isCircle ? 'rounded-full w-64 h-64 sm:w-80 sm:h-80' : 'rounded-[24px] w-64 sm:w-80' }}"
                     style="max-height:70vh">
            </div>
        </div>

        <div class="text-center space-y-1">
            <p class="text-white font-bold text-lg tracking-wide drop-shadow-lg">{{ $user->name }}</p>
