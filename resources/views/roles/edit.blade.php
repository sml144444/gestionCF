...
<div style="display:flex; align-items:center; gap:12px; margin-bottom:24px;">
    ...
    <div>
        <h1 style="font-size:18px; font-weight:800; color:#0f172a; margin:0;">
            Modifier : <span style="color:#0a6640;">{{ $role->name }}</span>
        </h1>
        <p style="font-size:11px; color:#64748b; margin:2px 0 0;">Modifiez le nom et les permissions du rôle</p>
    </div>
</div>
    ...
    <form method="POST" action="{{ route('roles.update', $role) }}">
        ...
        <div style="padding:22px 24px;">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px;">
                <label style="font-size:9px; font-weight:800; color:#94a3b8;
            ...
            @foreach($permissions as $group => $perms)
                ...
                <div style="border:1px solid #e2e8f0; border-radius:14px; overflow:hidden;">
                    <div style="padding:10px 16px; background:{{ $g['bg'] }};
                        ...
                        <button type="button" onclick="toggleGroup('{{ $group }}', '{{ $g['bg'] }}', '{{ $g['color'] }}')"
                    ...
                    <div style="padding:12px 16px; display:flex; flex-wrap:wrap; gap:8px; background:white;">
                        @foreach($perms as $perm)
                            @php
                                ...
                                $isChecked = in_array($perm->name, old('permission', $rolePermissions));
                            @endphp
                            <label style="display:inline-flex; align-items:center; gap:7px; cursor:pointer;
                                          padding:7px 12px; border-radius:10px; border:1.5px solid {{ $isChecked ? $g['color'].'60' : '#e2e8f0' }};
                                ...
                                <span class="perm-tick" style="width:16px; height:16px; border-radius:5px; flex-shrink:0;
