...
<div class="rc-wrap">
    ...
    <div class="rc-hero">
        ...
        <div>
            ...
            <p class="rc-hero-sub" style="text-transform:capitalize;">{{ $role->name }}</p>
        </div>
        @if($isSystem)
        ...
        <form action="{{ route('roles.update', $role) }}" method="POST">
            ...
            <div class="rc-name-field">
                ...
                <input
                    ...
                    {{ $isSystem ? 'readonly style="max-width:400px;background:#f8fafc;color:#94a3b8;"' : '' }}
                    required>
                @if($isSystem)
            ...
            <div style="padding:20px 24px 8px;">
                ...
                <div style="font-size:11px;color:#94a3b8;">{{ $role->permissions->count() }} permission(s) actuellement assignée(s)</div>
            </div>
                ...
                <div class="rc-perm-group">
                    <div class="rc-perm-group-header" onclick="toggleGroup('{{ $groupKey }}')">
                        <div class="rc-perm-group-label">
                            ...
                            @endphp
                            @if($activeCount > 0)
                                <span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:99px;background:{{ $group['bg'] }};color:{{ $group['color'] }};">{{ $activeCount }} actif</span>
                            @endif
                        </div>
                        <div style="display:flex;align-items:center;gap:8px;">
                            ...
                            <svg id="chevron-{{ $groupKey }}" width="16" height="16" fill="none" stroke="#94a3b8" viewBox="0 0 24 24" style="transition:.2s;transform:rotate({{ $activeCount > 0 ? '180' : '0' }}deg);">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                            </svg>
                        </div>
                    </div>
                    <div id="group-{{ $groupKey }}" class="rc-perm-group-body" style="{{ $activeCount === 0 ? 'display:none;' : '' }}">
                        @foreach($group['perms'] as $permName => $permLabel)
                            ...
                            @endphp
                            @if($exists)
                                <div class="rc-perm-item {{ $isChecked ? 'active' : '' }}" onclick="toggleCheck('{{ $cbId }}')">
                                    <input type="checkbox"
                                        id="{{ $cbId }}"
                                        name="permission[]"
                                        value="{{ $permName }}"
