...
<head>
    ...
    <title>503 — Maintenance | OFPPT</title>
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .status-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--green-600); box-shadow: 0 0 8px rgba(22,168,94,0.7); animation: blink 1.8s ease-in-out infinite; }
        ...
        /* Green-themed icon for maintenance (not an error, a planned event) */
        .shield-wrap { position: relative; width: 96px; height: 96px; margin: 0 auto 28px; }
        .shield-ring { position: absolute; inset: -12px; border-radius: 50%; border: 1px solid rgba(10,102,64,0.2); animation: spin 14s linear infinite; }
        .shield-ring::before { content: ''; position: absolute; top: -2px; left: 50%; transform: translateX(-50%); width: 6px; height: 6px; background: var(--green-600); border-radius: 50%; box-shadow: 0 0 8px rgba(22,168,94,0.7); }
        @keyframes spin { to { transform: rotate(360deg); } }
        .shield-icon { width: 96px; height: 96px; border-radius: 50%; background: linear-gradient(135deg, rgba(10,102,64,0.1), rgba(10,102,64,0.03)); border: 1.5px solid rgba(10,102,64,0.18); display: flex; align-items: center; justify-content: center; box-shadow: 0 0 24px rgba(10,102,64,0.1); }
        /* Gear spinning */
        .shield-icon svg { animation: gearSpin 10s linear infinite; transform-origin: center; }
        @keyframes gearSpin { to { transform: rotate(360deg); } }
        /* Green badge for 503 — maintenance is not an error */
        .code-badge { display: inline-flex; align-items: center; gap: 8px; background: rgba(10,102,64,0.07); border: 1px solid rgba(10,102,64,0.18); border-radius: 99px; padding: 5px 14px 5px 10px; margin-bottom: 20px; }
        .code-badge-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--green-700); }
        .code-badge-text { font-family: var(--font-mono); font-size: 10px; font-weight: 700; color: var(--green-800); letter-spacing: 2px; text-transform: uppercase; }
        ...
        /* Progress bar in OFPPT green */
        .progress-wrap { margin-bottom: 28px; }
        .progress-label { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
        .progress-text { font-size: 10px; font-family: var(--font-mono); color: var(--slate-500); letter-spacing: 1px; text-transform: uppercase; }
        .progress-track { width: 100%; height: 4px; background: rgba(0,0,0,0.06); border-radius: 99px; overflow: hidden; }
        .progress-bar { height: 100%; width: 65%; background: linear-gradient(90deg, var(--green-800), var(--green-600)); border-radius: 99px; animation: progressAnim 3s ease-in-out infinite alternate; }
        @keyframes progressAnim { from { width: 55%; } to { width: 78%; } }
    ...
    <div class="card">
        <div class="shield-wrap">
            ...
            <div class="shield-icon">
                <!-- Gear = maintenance, green because it's planned not a failure -->
                <svg width="40" height="40" fill="none" viewBox="0 0 24 24">
                    <path stroke="rgba(10,102,64,0.9)" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                    <path stroke="rgba(10,102,64,0.7)" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                </svg>
            </div>
        </div>
        ...
        <div class="code-badge">
            ...
            <span class="code-badge-text">MAINTENANCE 503</span>
        </div>

        <div class="title-403">503</div>
        <h1 class="title-main">Service temporairement indisponible</h1>
        <p class="desc">
            La plateforme est en <strong>maintenance programmée</strong>.<br>
            Nous travaillons à améliorer votre expérience. Le service sera <strong>rétabli très bientôt</strong>.
        </p>
        ...
        <div class="progress-wrap">
            <div class="progress-label">
                <span class="progress-text">Progression de la maintenance</span>
                <span class="progress-text" style="color:var(--green-700);">En cours…</span>
            </div>
        ...
        <div class="actions">
            ...
            <a href="mailto:support@ofppt.ma" class="btn-primary">
                <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
