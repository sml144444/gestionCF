...
.heures-hint strong { color:{{ $accent }}; }
</style>
                        ...
                        <div class="repl-panel">
                            <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:6px;">
                                <div style="display:flex;align-items:center;gap:6px;">
                                    ...
                                    <span style="font-size:10px;font-weight:700;color:#5b21b6;">{{ $module->remplacant->name }}</span>
                            ...
                            <div style="font-size:9px;color:#7c3aed;">
                                Remplace :
                                <span style="text-decoration:line-through;color:#94a3b8;margin-left:3px;">{{ $module->formateur->name ?? '—' }}</span>
                            </div>
                            @php $activeRecord = $replHistory->where('type','remplacement')->where('is_active', true)->first(); @endphp
                            @if($activeRecord)
                            ...
                            <div style="margin-top:8px;display:flex;flex-direction:column;gap:4px;">
                                @foreach($pastReplacements->sortByDesc('start_date') as $record)
                                    <div class="repl-history-row">
                                        <div>
                                            <span style="font-size:10px;font-weight:700;color:#1e293b;">{{ $record->formateur->name ?? '—' }}</span>
                                        </div>
                                        <div style="text-align:right;flex-shrink:0;">
                                            <div style="font-size:9px;color:#64748b;">
                                                {{ $record->start_date->format('d/m/Y') }} → {{ $record->end_date ? $record->end_date->format('d/m/Y') : 'en cours' }}
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
        ...
        <form method="POST" action="{{ route('modules.store') }}" style="display:flex;flex-direction:column;gap:14px;">
            ...
            <div>
                ...
                <select name="id_filiere" id="create-id-filiere" class="mod-input" required
                    ...
                    @foreach($filieres as $f)
                        <option value="{{ $f->id }}" data-duree="{{ (int)$f->duree }}">{{ $f->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            {{-- Année (filtered by filière duree) --}}
            <div>
                <label class="mod-label">Année <span style="color:#ef4444;">*</span></label>

                {{-- Info banner shown only when filière has duree < 3 --}}
...
{{-- ════════════════════════════════════════════════════════════
     MODAL ÉDITER
     NOTE: id_user_remplacant excluded — use the replacement panel on the card
════════════════════════════════════════════════════════════ --}}
@if($canEdit)
            ...
            <div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
                    ...
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:9px 12px;border-radius:10px;border:1.5px solid #e2e8f0;background:white;font-size:12px;font-weight:600;color:#475569;">
                        <input type="radio" name="annee" value="{{ $val }}" id="edit-annee-{{ $val }}"
