...
@php
    ...
    $badgeStyles = [
        ...
        'formateur'    => 'bg-teal-100 text-teal-800',
        'stagiaire'    => 'bg-blue-100 text-blue-700',
    ];
...
@section('sidebar-nav')
    ...
    <x-nav-item route="#" label="Gestion users"
        icon="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
    
    <x-nav-section label="Accès complet" />
    {{-- Had l-items ankhaliwhom kifma homa hit icons kit-generaw b text-current --}}
...
@section('content')
    {{-- Header m3a border khdar --}}
    <div class="bg-white rounded-2xl border-l-4 border-emerald-500 px-6 py-5 mb-5 shadow-sm">
        <h2 class="text-lg font-semibold text-slate-800">Bonjour, {{ Auth::user()->name }} 👋</h2>
        <p class="text-sm text-slate-500 mt-1">Accès complet à la plateforme OFPPT</p>
    </div>

    {{-- Stats Cards m3a text emerald --}}
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        @foreach([['Utilisateurs','—'],['Stagiaires','—'],['Formateurs','—']] as [$l,$v])
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <p class="text-xs text-slate-400 uppercase tracking-widest mb-2">{{ $l }}</p>
            <p class="text-2xl font-semibold text-emerald-700">{{ $v }}</p>
        </div>
        @endforeach
    </div>
