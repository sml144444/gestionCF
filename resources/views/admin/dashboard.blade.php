...
<div class="bg-gradient-to-r from-emerald-700 to-emerald-500 rounded-2xl px-6 py-5 mb-6 shadow-sm flex items-center justify-between">
    ...
    <div class="hidden sm:flex items-center gap-2 bg-white/15 rounded-xl px-4 py-2">
        ...
        <span class="text-sm text-white font-medium">{{ now()->isoFormat('dddd D MMMM YYYY') }}</span>
    </div>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
        ...
        <p class="text-2xl font-bold text-slate-800">{{ $stats['formateurs'] ?? '—' }}</p>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    ] as [$label, $val, $textColor, $bg])
    <div class="{{ $bg }} rounded-2xl border border-slate-200 p-4 shadow-sm flex items-center justify-between">
        <div>
            <p class="text-xs text-slate-400 uppercase tracking-wide mb-1">{{ $label }}</p>
            <p class="text-xl font-bold {{ $textColor }}">{{ $val }}</p>
        </div>
    </div>
    @endforeach
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-5 grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            ] as [$label, $route, $icon, $hoverClass])
            <a href="{{ $route }}"
               class="flex flex-col items-center gap-2 rounded-xl border border-slate-200 p-4 text-center text-slate-500 transition-all {{ $hoverClass }} group">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-5 space-y-3">
            ...
            <div class="flex items-center justify-between py-2 border-b border-slate-50">
                ...
                <span class="text-xs font-semibold {{ app()->isProduction() ? 'text-emerald-600' : 'text-amber-600' }}">
                    {{ ucfirst(app()->environment()) }}
                </span>
            </div>
            <div class="flex items-center justify-between py-2 border-b border-slate-50">
                <span class="text-xs text-slate-400">PHP</span>
                <span class="text-xs font-semibold text-slate-600">{{ PHP_VERSION }}</span>
            </div>
