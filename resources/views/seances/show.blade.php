@extends('layouts.app')
@section('title', 'Séance — ' . ($emploi->module?->name ?? 'Module'))
...
<div class="flash-ok">
    ...
    <p style="font-size:13px;font-weight:600;color:var(--ac-tx);margin:0;">{{ session('success') }}</p>
</div>
@endif
@if(session('error'))
<div class="flash-err">
...
<div class="hero">
    <div style="display:flex;align-items:center;gap:16px;">
        ...
        <div>
            <h1 class="hero-title">{{ $emploi->module?->name ?? 'Séance' }}</h1>
            <p class="hero-sub">
    ...
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        ...
        <a href="{{ route('emplois.index') }}" class="hero-back">← Retour</a>
    </div>
</div>
...
<div class="info-grid">
    <div class="info-card">
        ...
        <div class="info-val">{{ $emploi->date_debut->format('d/m/Y') }}</div>
        <div class="info-sub">{{ $emploi->date_debut->translatedFormat('l') }}</div>
    </div>
    <div class="info-card">
        <div class="info-label">🕐 Horaire</div>
        <div class="info-val">{{ $emploi->date_debut->format('H:i') }} – {{ $emploi->date_fin->format('H:i') }}</div>
        <div class="info-sub">{{ count($activeParts) }} demi-séance(s) × 2.5h = {{ $totalDuree }}h</div>
    </div>
    @if($emploi->salle)
    <div class="info-card">
        <div class="info-label">🏫 Salle</div>
        <div class="info-val">{{ $emploi->salle->name }}</div>
        @if($emploi->salle->capacite)
            <div class="info-sub">Capacité: {{ $emploi->salle->capacite }} pers.</div>
        @endif
    </div>
    @endif
    @if($emploi->gestionnaire)
    <div class="info-card">
        <div class="info-label">👤 Formateur</div>
        <div class="info-val">{{ $emploi->gestionnaire->name }}</div>
        <div class="info-sub">{{ $emploi->gestionnaire->email }}</div>
    </div>
    @endif
    @if($emploi->remplacant)
    <div class="info-card">
        <div class="info-label">🔄 Remplaçant</div>
        <div class="info-val">{{ $emploi->remplacant->name }}</div>
        <div class="info-sub">{{ $emploi->remplacant->email }}</div>
    </div>
    @endif
    <div class="info-card">
        ...
        <div class="info-val">
            @if($isStagiaire)
                {{ $emploi->groupe ? \App\Models\User::where('id_groupe', $emploi->id_groupe)->where('role','stagiaire')->count() : '—' }}
            @else
                {{ $stagiaires->count() }}
            @endif
        </div>
        <div class="info-sub">{{ $emploi->groupe?->name ?? '—' }}</div>
    </div>
</div>
...
<div class="section">
        ...
        <div>
            ...
            <div class="section-sub">
                ...
                @else
                    ...
                    Cette séance a <strong>{{ count($activeParts) }} demi-séance(s)</strong>
                    ({{ implode(', ', array_map('strtoupper', $activeParts)) }}) × 2.5h = <strong>{{ $totalDuree }}h</strong>.
                @endif
            </div>
        </div>
        @if($emploi->statut !== 'annule' && $canPresence && $stagiaires->isNotEmpty())
    ...
    {{-- ── Stagiaire self-view info banner ── --}}
    @if($isStagiaire)
    ...
    @elseif(!$canPresence)
        {{-- ── READ-ONLY VIEW (stagiaire / read-only role) ── --}}
        <div style="overflow-x:auto;">
        <table class="ptbl">
            <thead>
                <tr>
                    <th class="th-name">
                        @if($isStagiaire) Votre nom @else Stagiaire @endif
                    </th>
                    @foreach($activeParts as $part)
                        <th class="th-part"
                            style="background:{{ $partConfig[$part]['bg'] }};
                                   color:{{ $partConfig[$part]['color'] }};
                                   border-left:2px solid {{ $partConfig[$part]['th'] }};">
                            {{ strtoupper($part) }}<br>
                ...
                <tr class="{{ $absCount > 0 ? 'row-has-absence' : '' }}">
                    <td class="td-name">
                        <div style="display:flex;align-items:center;gap:8px;">
                            <div class="avatar">
                                {{ strtoupper(mb_substr($stagiaire->name, 0, 1) . mb_substr(explode(' ', $stagiaire->name)[1] ?? '', 0, 1)) }}
                            </div>
                            <div>
...
<td class="td-bilan">
    ...
    @else
        {{-- ── EDITABLE VIEW (admin / gestionnaire / formateur) ── --}}
        ...
        <div class="select-all-bar">
            ...
            @foreach($activeParts as $part)
                <button type="button"
                        ...
                        style="color:{{ $partConfig[$part]['color'] }};border-color:{{ $partConfig[$part]['border'] }};"
                        onclick="toggleAllPart('{{ $part }}', true)">
                    Tous absents {{ strtoupper($part) }}
                </button>
                <button type="button"
                        class="btn-select-all"
                        style="color:#64748b;border-color:#e2e8f0;"
                        onclick="toggleAllPart('{{ $part }}', false)">
                    Tous présents {{ strtoupper($part) }}
                </button>
            @endforeach
        </div>
        ...
        <form id="presence-form"
              method="POST"
              action="{{ route('seances.presence', $emploi) }}"
              style="{{ $emploi->statut === 'annule' ? 'pointer-events:none;opacity:.55;' : '' }}">
        @csrf
                    ...
                    <td class="td-part"
                        ...
                        <div class="abs-checkbox-wrap">
                            <input type="checkbox"
                                   class="abs-cb"
                                   id="cb_{{ $i }}_{{ $part }}"
                                   name="presences[{{ $i }}][{{ $part }}]"
                                   value="1"
                                   data-idx="{{ $i }}"
                                   data-part="{{ $part }}"
                                   onchange="updateBilan({{ $i }})"
                                   {{ $isAbsent[$part] ? 'checked' : '' }}>
                            <label for="cb_{{ $i }}_{{ $part }}"
...
<div id="bilan_{{ $i }}">
    ...
    @endif
        ...
        <div class="legend">
            ...
            @foreach($activeParts as $part)
                <span class="legend-item" style="color:{{ $partConfig[$part]['color'] }};">
                    <span style="width:10px;height:10px;border-radius:3px;background:{{ $partConfig[$part]['bg'] }};border:1.5px solid {{ $partConfig[$part]['color'] }};display:inline-block;"></span>
                    {{ strtoupper($part) }} = 2.5h
                </span>
            @endforeach
        </div>
...
<div class="section">
    <div class="section-head">
        <div>
            ...
            <div class="section-sub">{{ $coursItems->count() }} document(s) partagé(s)</div>
        </div>
    </div>

    @forelse($coursItems as $doc)
        @php
            $icon   = match(true) {
        ...
        @endphp
        <div class="res-item">
            <div class="res-icon" style="background:{{ $iconBg }};">{{ $icon }}</div>
            <div style="flex:1;min-width:0;">
                <div class="res-title">{{ $doc->titre }}</div>
                @if($doc->description)
                    <div class="res-desc">{{ Str::limit($doc->description, 120) }}</div>
                @endif
                @if(!empty($doc->lien))
                    <a href="{{ $doc->lien }}" target="_blank" class="res-link" style="margin-top:4px;display:inline-block;">
                        🔗 Ouvrir le lien
                    </a>
                @endif
                @if(!empty($doc->fichier[0]))
                    <a href="{{ Storage::url($doc->fichier[0]) }}" target="_blank" class="res-link" style="margin-top:4px;display:inline-block;">
                        ⬇ Télécharger
                    </a>
                @endif
                <div style="font-size:9px;color:#cbd5e1;margin-top:4px;">
                    Ajouté {{ $doc->created_at?->diffForHumans() }}
                    @if($doc->formateur) par {{ $doc->formateur->name }} @endif
                </div>
            </div>
            @if($canEditClassroom)
                <form method="POST"
                      action="{{ route('seances.ressource.destroy', [$emploi, $doc]) }}"
                      onsubmit="return confirm('Supprimer « {{ addslashes($doc->titre) }} » ?')">
                    @csrf @method('DELETE')
    ...
    <div class="add-form">
        ...
        <form method="POST"
              action="{{ route('seances.ressource.store', $emploi) }}"
...
<script>
const HALF_DUREE   = {{ $halfDuree }};
const ACTIVE_PARTS = @json($activeParts);
