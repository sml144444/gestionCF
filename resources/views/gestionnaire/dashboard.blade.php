...
<div class="relative overflow-hidden rounded-2xl mb-6 bg-[#1e293b]">
    ...
    <div class="relative px-6 py-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-400 text-sm mt-1">{{ now()->isoFormat('dddd D MMMM YYYY') }}</p>
        </div>
        ...
        <a href="{{ route('emplois.index') }}"
           class="flex items-center gap-2 bg-amber-400 text-amber-900 rounded-xl px-4 py-2.5 text-xs font-bold hover:bg-amber-300 transition-colors shadow-lg">
            ...
            {{ $stats['seances_brouillon'] }} à publier
        </a>
        @endif
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="lg:col-span-2 grid gap-5">
        ...
        ] as [$title, $count, $route, $color, $bg, $border, $icon, $desc])
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-50 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:{{ $bg }};">
                        <svg class="w-4 h-4" fill="none" stroke="{{ $color }}" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $icon }}"/>
                        </svg>
                    </div>
                ...
                <a href="{{ $route }}" class="text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1">
            ...
            <div class="p-5 flex items-center gap-5">
                <div class="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 font-black text-2xl"
                     style="background:{{ $bg }}; color:{{ $color }}; border:2px solid {{ $border }};">
                    {{ $count }}
                </div>
                <div class="flex-1">
                    <p class="text-sm font-bold text-slate-700">
                        {{ $count === 0 ? 'Aucune en attente ✓' : $count.' assignée à vous' }}
                    </p>
