...
.filter-select, .filter-input { font-size:12px; font-weight:500; padding:8px 12px; border:1.5px solid #e2e8f0; border-radius:10px; background:white; color:#1e293b; outline:none; cursor:pointer; transition:border-color .15s; }
...
table.abs-table thead th { padding:11px 16px; font-size:10px; font-weight:800; color:#94a3b8; text-transform:uppercase; letter-spacing:.8px; background:#f8fafc; border-bottom:1px solid #e2e8f0; text-align:left; white-space:nowrap; }
...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div><div class="stat-val" style="color:#1e293b;">{{ $stats['total'] }}</div><div class="stat-lbl">Total</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon" style="background:#fee2e2;">❌</div>
        <div><div class="stat-val" style="color:#dc2626;">{{ $stats['absences'] }}</div><div class="stat-lbl">Absences</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon" style="background:#fef3c7;">⏰</div>
        <div><div class="stat-val" style="color:#d97706;">{{ $stats['retards'] }}</div><div class="stat-lbl">Retards</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon" style="background:#d1fae5;">✅</div>
        <div><div class="stat-val" style="color:#059669;">{{ $stats['justifies'] }}</div><div class="stat-lbl">Justifiés</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon" style="background:#fce7f3;">⚠️</div>
        <div><div class="stat-val" style="color:#be185d;">{{ $stats['injustifies'] }}</div><div class="stat-lbl">Non justifiés</div></div>
    </div>
</div>
        ...
        <table class="abs-table">
            ...
            @foreach($absences as $abs)
                ...
                <tr>
                    ...
                    <td>
                        @php
                            $heures = $abs->duree ?? ($emploi
                                ? round($emploi->date_debut->diffInMinutes($emploi->date_fin) / 60, 1)
                                : null);
                        @endphp
                        ...
                        @if($canJustify)
                            @if($abs->file_justification)
                                <div style="display:flex;flex-direction:column;gap:6px;">
                                    ...
                                    <form method="POST"
                                          action="{{ route('absences.fichier.delete', $abs) }}"
