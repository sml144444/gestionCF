...
<div class="fil-grid">
    ...
    <a href="{{ route('stagiaire.index', ['filiere_id' => $filiere->id]) }}"
       class="fil-card">
        ...
        <div class="fil-card-header">
            <div style="flex:1;min-width:0;">
                <p class="fil-card-title">{{ $filiere->name }}</p>
                <p class="fil-card-subtitle">
                    {{ $filiere->duree }} an{{ $filiere->duree > 1 ? 's' : '' }} de formation
                    &nbsp;·&nbsp; {{ $grps->count() }} groupe{{ $grps->count() > 1 ? 's' : '' }}
                </p>
            </div>
            <div class="fil-count-badge">
                <span class="fil-count-num">{{ $total }}</span>
        ...
        <div class="fil-occ-row">
            <div class="fil-occ-labels">
                <span>{{ $total }} / {{ $cap }} places occupées</span>
                <strong style="color:{{ $occClr }};">{{ $occ }}%</strong>
            </div>
            <div class="occ-bar">
                <div class="occ-fill" style="width:{{ $occ }}%;background:{{ $occClr }};"></div>
            </div>
        </div>
            ...
            @if($grps1->isNotEmpty())
                ...
                <div class="fil-grp-chips">
                    ...
                    <span class="fil-grp-chip">
                        {{ $g->name }}
                        <span class="fil-grp-count">{{ $g->stagiaires_count }}</span>
                    </span>
                    @endforeach
                </div>
            @endif
        ...
        <div class="fil-card-footer">
            <span class="fil-footer-text">
                {{ $cap - $total }} place{{ ($cap - $total) > 1 ? 's' : '' }} libre{{ ($cap - $total) > 1 ? 's' : '' }}
            </span>
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
        ...
        <div style="padding:8px 14px;border-radius:12px;background:#f8fafc;
            ...
            <div style="font-size:9px;font-weight:700;color:#64748b;
                        text-transform:uppercase;letter-spacing:.5px;">Grp An.{{ $yr }}</div>
        </div>
        @endforeach
    </div>
</div>
