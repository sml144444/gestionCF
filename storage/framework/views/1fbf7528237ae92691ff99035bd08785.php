...
<div class="rc-hero">
    ...
    <div class="rc-stats">
        <div class="rc-stat">
            <div class="rc-stat-val"><?php echo e($stats['total']); ?></div>
            <div class="rc-stat-lbl">Total</div>
        </div>
        <div class="rc-stat">
            <div class="rc-stat-val"><?php echo e($stats['en_attente']); ?></div>
            <div class="rc-stat-lbl">En attente</div>
        </div>
        <div class="rc-stat">
            <div class="rc-stat-val"><?php echo e($stats['traitee']); ?></div>
...
<div class="rc-filters">
    <form method="GET" action="<?php echo e(route('reclamations.index')); ?>" style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;width:100%;">
        <select name="status" class="rc-filter-select" onchange="this.form.submit()">
            ...
            <option value="traitee"    <?php echo e(request('status') === 'traitee'    ? 'selected' : ''); ?>>✅ Traitées</option>
        </select>
        <select name="type" class="rc-filter-select" onchange="this.form.submit()">
            <option value="">Tous les types</option>
            <option value="note"      <?php echo e(request('type') === 'note'      ? 'selected' : ''); ?>>📝 Note</option>
            <option value="absence"   <?php echo e(request('type') === 'absence'   ? 'selected' : ''); ?>>📅 Absence</option>
            <option value="emploi"    <?php echo e(request('type') === 'emploi'    ? 'selected' : ''); ?>>🗓️ Emploi</option>
            <option value="formateur" <?php echo e(request('type') === 'formateur' ? 'selected' : ''); ?>>👨‍🏫 Formateur</option>
            <option value="autre"     <?php echo e(request('type') === 'autre'     ? 'selected' : ''); ?>>📌 Autre</option>
        </select>
        <?php if(request()->hasAny(['status','type'])): ?>
            <a href="<?php echo e(route('reclamations.index')); ?>" class="btn-sm btn-sm-danger">✕ Réinitialiser</a>
        <?php endif; ?>
    </form>
</div>
...
<div class="rc-table-wrap">
    <?php $__empty_1 = true; $__currentLoopData = $reclamations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        ...
        <tr>
            <td>
                <div style="font-weight:700;font-size:13px;"><?php echo e($rec->stagiaire->name ?? '—'); ?></div>
                <div style="font-size:11px;color:#94a3b8;"><?php echo e($rec->stagiaire->email ?? ''); ?></div>
            </td>
            <td>
            ...
            <td style="font-size:11px;color:#64748b;white-space:nowrap;">
                <?php echo e($rec->created_at->format('d/m/Y')); ?><br>
                <span style="color:#94a3b8;"><?php echo e($rec->created_at->format('H:i')); ?></span>
            </td>
            <td>
                <?php if($rec->status === 'en_attente'): ?>
            ...
            <td>
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    <?php if($rec->status === 'en_attente'): ?>
                        <form method="POST" action="<?php echo e(route('reclamations.status', $rec)); ?>">
                    ...
                    <?php if(Auth::user()->role === 'admin'): ?>
                        <button onclick="openDeleteModal('<?php echo e(route('reclamations.destroy', $rec)); ?>')" class="btn-sm btn-sm-danger">🗑️</button>
                    <?php endif; ?>
                </div>
            </td>
        </tr>
