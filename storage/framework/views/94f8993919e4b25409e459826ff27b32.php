...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex items-center gap-5">
        <div class="w-16 h-16 <?php echo e($rc['shape']); ?> <?php echo e($rc['bg']); ?> flex items-center justify-center
                    text-white text-2xl font-bold flex-shrink-0 overflow-hidden shadow-md">
            <?php if(!$isStagiaire && $user->photo): ?>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex gap-2 mt-2 flex-wrap">
                ...
                <span class="text-xs px-2.5 py-1 rounded-full font-semibold border <?php echo e($rc['badge']); ?> <?php echo e($rc['border']); ?>">
                ...
                <?php else: ?>
                    ...
                    <?php endif; ?>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('user-manage-formateur')): ?>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('user-manage-gestionnaire')): ?>
    ...
    <div class="grid grid-cols-1 <?php echo e(!$isStagiaire ? 'md:grid-cols-2' : ''); ?> gap-5">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <dl class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    <dd class="mt-1 font-medium text-slate-800"><?php echo e($user->date_naissance?->format('d/m/Y') ?? '—'); ?></dd>
                </div>
                <div>
                    <dt class="text-xs text-slate-400 uppercase font-semibold tracking-wider">Absences</dt>
                    <dd class="mt-1 font-semibold text-slate-800"><?php echo e($user->absences->count()); ?> absence(s)</dd>
                </div>
            </dl>
            <?php else: ?>
            ...
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                    ...
                    <dd class="font-semibold text-slate-800"><?php echo e($user->name); ?></dd>
                </div>
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                    <dt class="text-xs text-slate-400 uppercase font-semibold tracking-wider">Email</dt>
                    <dd class="font-medium text-slate-700 truncate max-w-[200px]"><?php echo e($user->email); ?></dd>
                </div>
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                    <dt class="text-xs text-slate-400 uppercase font-semibold tracking-wider">Téléphone</dt>
                    <dd class="font-medium text-slate-800"><?php echo e($user->phone ?? '—'); ?></dd>
                </div>
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                    <dt class="text-xs text-slate-400 uppercase font-semibold tracking-wider">CIN</dt>
                    <dd class="font-medium text-slate-800"><?php echo e($user->cin ?? '—'); ?></dd>
                </div>
                <div class="flex justify-between items-center py-2">
                    <dt class="text-xs text-slate-400 uppercase font-semibold tracking-wider">Date de naissance</dt>
                    <dd class="font-medium text-slate-800"><?php echo e($user->date_naissance?->format('d/m/Y') ?? '—'); ?></dd>
                </div>
            </dl>
            <?php endif; ?>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                    ...
                    <dd class="font-medium text-slate-800"><?php echo e($user->matricule_formateur ?? '—'); ?></dd>
                </div>
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                    <dt class="text-xs text-slate-400 uppercase font-semibold tracking-wider">Date d'embauche</dt>
                    <dd class="font-medium text-slate-800"><?php echo e($user->date_embauche?->format('d/m/Y') ?? '—'); ?></dd>
                </div>
                <?php if($isFormateur): ?>
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                ...
                <div class="flex justify-between items-center py-2 border-b border-slate-50">
                    ...
                    <dd class="font-semibold text-slate-800"><?php echo e($user->modules->count()); ?> module(s)</dd>
                </div>
                <?php endif; ?>
                <div class="flex justify-between items-center py-2">
                    <dt class="text-xs text-slate-400 uppercase font-semibold tracking-wider">Membre depuis</dt>
                    <dd class="font-medium text-slate-800"><?php echo e($user->created_at->format('d/m/Y')); ?></dd>
                </div>
            </dl>
        </div>
        <?php endif; ?>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <h3 class="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
            ...
            Modules enseignés
            <span class="ml-auto text-xs font-normal text-slate-400"><?php echo e($user->modules->count()); ?> module(s)</span>
        </h3>
        <div class="flex flex-wrap gap-2">
            ...
            <span class="text-xs font-semibold px-3 py-1.5 rounded-full
                         <?php echo e($rc['light']); ?> <?php echo e($rc['text']); ?> border <?php echo e($rc['border']); ?>">
                <?php echo e($module->name); ?>

                <span class="text-[10px] opacity-60 ml-1"><?php echo e($module->nbr_heure); ?>h</span>
            </span>
    ...
    <?php if($isStagiaire && $user->absences->isNotEmpty()): ?>
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <h3 class="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
            ...
            <span class="ml-auto text-xs font-normal text-slate-400">
                <?php echo e($user->absences->count()); ?> total
                · <?php echo e($user->absences->where('justifiee', true)->count()); ?> justifiée(s)
            </span>
        </h3>
        <div class="flex gap-4 text-center">
            <div class="flex-1 bg-red-50 rounded-xl p-3">
                <p class="text-2xl font-bold text-red-600"><?php echo e($user->absences->count()); ?></p>
                <p class="text-xs text-red-400 mt-1">Total</p>
            </div>
            <div class="flex-1 bg-emerald-50 rounded-xl p-3">
                <p class="text-2xl font-bold text-emerald-600"><?php echo e($user->absences->where('justifiee', true)->count()); ?></p>
                <p class="text-xs text-emerald-400 mt-1">Justifiées</p>
            </div>
            <div class="flex-1 bg-amber-50 rounded-xl p-3">
                <p class="text-2xl font-bold text-amber-600"><?php echo e($user->absences->where('justifiee', false)->count()); ?></p>
